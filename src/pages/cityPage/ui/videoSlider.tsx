...
const VideoSlider = ({ items = [] }: VideoSliderProps) => {
  ...
  if (isVerySmallScreen) {
    return (
      ...
      >
        {items.map((item) => (
          <VideoItemWithLoader key={item.id} item={item} />
        ))}
      </Box>
    );
  }
...
const VideoItemWithLoader = ({ item }: { item: VideoItem }) => {
  const theme = useTheme();
  const { data, isLoading } = useGetFileByIdQuery(item.videoId || "", {
    skip: !item.videoId,
  });
  ...
  if (isLoading) {
    return (
      <Box
        sx={{
          height: { xs: "200px", sm: "300px", md: "360px" },
          display: "flex",
  ...
  return (
    ...
    >
      ...
      >
        <source src={`data:video/mp4;base64,${data.data}`} type="video/mp4" />
      </video>
